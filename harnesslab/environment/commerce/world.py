...
class CommerceWorld(Environment):
    ...
    def list_tools(self) -> list[ToolSpec]:
        return [
            ToolSpec(name="get_customer", description="Get a customer by id.", parameters={"customer_id": {"type": "string"}}, permission="customer_read"),
            ToolSpec(name="search_customers", description="Search customers by name or email substring.", parameters={"query": {"type": "string"}}, permission="customer_read"),
            ToolSpec(name="get_order", description="Get an order by id.", parameters={"order_id": {"type": "string"}}, permission="order_read"),
            ToolSpec(name="list_orders", description="List orders for a customer_id, newest first.", parameters={"customer_id": {"type": "string"}}, permission="order_read"),
            ToolSpec(name="search_orders", description="Search orders by customer_id and optional status.", parameters={"customer_id": {"type": "string"}, "status": {"type": "string"}}, permission="order_read"),
            ToolSpec(name="get_payment", description="Get a payment by id or by order_id.", parameters={"payment_id": {"type": "string"}, "order_id": {"type": "string"}}, permission="payment_read"),
            ToolSpec(name="check_refund_eligibility", description="Return whether an order can be refunded and why.", parameters={"order_id": {"type": "string"}}, permission="payment_read"),
            ToolSpec(name="refund_payment", description="Refund the captured payment for an order if eligible.", parameters={"order_id": {"type": "string"}}, permission="refund"),
            ToolSpec(name="get_ticket", description="Get a support ticket by id.", parameters={"ticket_id": {"type": "string"}}, permission="ticket_write"),
            ToolSpec(name="create_ticket", description="Create a support ticket.", parameters={"customer_id": {"type": "string"}, "subject": {"type": "string"}, "order_id": {"type": "string"}}, permission="ticket_write"),
            ToolSpec(name="update_ticket", description="Update ticket status or note.", parameters={"ticket_id": {"type": "string"}, "status": {"type": "string"}, "note": {"type": "string"}}, permission="ticket_write"),
            ToolSpec(name="send_message", description="Send a message to a customer.", parameters={"customer_id": {"type": "string"}, "body": {"type": "string"}}, permission="message_send"),
            ToolSpec(name="search_policy", description="Search policy documents.", parameters={"query": {"type": "string"}}, permission="policy_read"),
        ]
    ...
    def call_tool(self, name: str, arguments: dict[str, Any] | None = None) -> ToolResult:
        ...
        if spec.permission and not self.permissions.get(spec.permission, False):
            return ToolResult(ok=False, code="permission_denied", error=f"Permission denied: {spec.permission}")
    ...
    def inject_event(self, kind: str, payload: dict[str, Any]) -> None:
        if kind == "order_status":
            ...
            self._state["messages"].append(
                {"id": f"m_{len(self._state['messages'])+1}", "customer_id": payload["customer_id"], "direction": "inbound", "body": payload.get("body", "")}
            )
        else:
    ...
    def _maybe_fault(self, tool: str) -> ToolResult | None:
        for f in self.faults:
            if f.get("tool") not in (None, "*", tool):
                continue
            if self._rng.random() > float(f.get("probability", 1.0)):
                continue
    ...
    def _search_customers(self, query: str) -> list[dict[str, Any]]:
        q = query.lower()
        return [c for c in self._state["customers"].values() if q in c["name"].lower() or q in c["email"].lower()]
    ...
    def _check_refund_eligibility(self, order_id: str) -> dict[str, Any]:
        ...
        return {"order_id": order_id, "eligible": ok, "reason": reason, "amount": order["amount"]}
    ...
    def _refund_payment(self, order_id: str) -> dict[str, Any]:
        ...
        rid = f"r_{order_id}"
        refund = {"id": rid, "order_id": order_id, "payment_id": pay["id"], "amount": pay["amount"], "status": "completed"}
    ...
    def _create_ticket(self, customer_id: str, subject: str, order_id: str | None = None) -> dict[str, Any]:
        self._req("customers", "customer", customer_id)
        tid = f"t_{len(self._state['tickets']) + 1}"
        ticket = {"id": tid, "customer_id": customer_id, "order_id": order_id, "status": "open", "subject": subject}
        self._state["tickets"][tid] = ticket
        return ticket

    def _update_ticket(self, ticket_id: str, status: str | None = None, note: str | None = None) -> dict[str, Any]:
    ...
    def _send_message(self, customer_id: str, body: str) -> dict[str, Any]:
        self._req("customers", "customer", customer_id)
        msg = {"id": f"m_{len(self._state['messages']) + 1}", "customer_id": customer_id, "direction": "outbound", "body": body}
        self._state["messages"].append(msg)
        return msg

    def _search_policy(self, query: str) -> list[dict[str, Any]]:
        q = query.lower()
        return [p for p in self._state["policies"].values() if q in p["title"].lower() or q in p["text"].lower() or q in p["id"].lower()]